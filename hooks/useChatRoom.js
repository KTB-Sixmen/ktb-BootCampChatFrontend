...
export const useChatRoom = () => {
  ...
  const [answerCheckModalOpen, setAnswerCheckModalOpen] = useState(false);
  const [answerCheckMessage, setAnswerCheckMessage] = useState(null);
  ...
  // useChatRoom.js 내 수정 예시
  useEffect(() => {
    // 소켓이 연결되었고 socketRef.current가 유효할 때만 이벤트 등록
    if (!connected || !socketRef.current) {
      console.log("소켓이 연결되지 않았거나 socketRef.current가 없습니다.");
      return;
    }

    // const handleAnswerCheck = ({ correct, username }) => {
    //   console.log("answerCheck 이벤트 발생:", correct, username);
    //   if (correct) {
    //     alert("정답입니다!");
    //   } else {
    //     alert("틀렸습니다!");
    ...
    const handleAnswerCheck = ({ correct, username }) => {
      console.log("answerCheck 이벤트 발생:", correct, username);
      if (correct) {
        setAnswerCheckMessage("정답입니다!");
      } else {
        setAnswerCheckMessage("틀렸습니다!");
      }
      setAnswerCheckModalOpen(true);
    };

    socketRef.current.on("answerCheck", handleAnswerCheck);

    // cleanup: 컴포넌트 언마운트 또는 재렌더링 시 이벤트 제거
    return () => {
      if (socketRef.current) {
        socketRef.current.off("answerCheck", handleAnswerCheck);
        console.log("answerCheck 이벤트 리스너 해제");
      }
    };
  }, [connected, socketRef]);
  ...
  return {
    ...
    answerCheckModalOpen,
    answerCheckMessage,
    setAnswerCheckModalOpen,
