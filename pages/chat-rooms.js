...
function ChatRoomsComponent() {
  ...
  const handlePasswordChange = (roomId, value) => {
    setPasswords((prev) => {
      ...
      passwordsRef.current = updated; // 항상 최신 상태를 ref에 저장
      return updated;
    });
  };
  ...
  const handleJoinRoom = async (roomId) => {
    ...
    const password = passwordsRef.current[roomId] || "";
    ...
    try {
      ...
      if (error.status === 404) {
        ...
        errorMessage =
          "채팅방 입장 권한이 없습니다.(올바른 비밀번호를 입력해주세요.)";
      }
  ...
  const columns = useMemo(
    () => [
      ...
      {
        accessorKey: "passwordInput",
        header: "비밀번호",
        cell: cellHelper(({ rowData }) => {
          return rowData.hasPassword ? (
            <input
              value={passwords[rowData._id]}
              onChange={(e) => {
                const inputValue = e.target.value;
                handlePasswordChange(rowData._id, inputValue);
              }}
              placeholder="비밀번호 입력"
