...
import { AlertCircle, PartyPopper, WifiOff } from "lucide-react";
...
const ChatPage = () => {
  ...
  return (
    <div className="chat-container">
      {/* answerCheck 이벤트에 따른 Modal 표시 */}
      <Modal
        isOpen={answerCheckModalOpen}
        toggle={() => setAnswerCheckModalOpen(false)}
        type="center"
        size="md"
        direction="vertical"
      >
        <ModalHeader toggle={() => setAnswerCheckModalOpen(false)}>
        ...
        <ModalFooter>
          <Button
            variant="primary"
            size="lg"
            onClick={() => setAnswerCheckModalOpen(false)}
            className="w-full"
          >
            닫기
          </Button>
